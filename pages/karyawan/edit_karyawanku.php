...
include '././koneksi.php';
$no = $_GET['no'];
        $sql = mysqli_query($koneksi,"select * from paket where id_paket='$no'");
...
if(!empty($id_paket) and !empty($nama_paket) and !empty($keterangan) and !empty($tgl) and !empty($harga)) {
  $sql = mysqli_query($koneksi,"update paket set id_paket = '$id_paket', nama_paket = '$keterangan', keterangan = '$keterangan', tgl_masuk='$tgl', harga = '$harga', id_petugas = '$pet'")
if ($sql) {
